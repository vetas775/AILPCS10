...
$(function(){if('fReeLayoutMain' == locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
...
function getHeadMsg(){
	$.ajax({
		type: "POST",
		url: locat+'/headPanel/fReeGetNecessaryList.do?tm='+new Date().getTime(),
    	data: encodeURI(""),
		dataType:'json',
		//beforeSend: validateData,
		cache: false,
		success: function(data){
			 $.each(data.resList, function(i, resList){
				 $("#user_info").html('<small>欢迎</small> '+resList.NAME+'');//登陆者资料
				 user = resList.USERNAME;
				 USER_ID = resList.USER_ID;		//用户ID
				 if(resList.USERNAME != 'admin'){
					 $("#systemset").hide();	//隐藏系统设置
				 }
			 });
			 updateUserPhoto(data.userPhoto);			//显示/刷新用户头像
			 fhsmsCount = Number(data.innerMailCount);
...
function getFhsmsCount(){
	$.ajax({
		type: "POST",
		url: locat+'/headPanel/countInnerMailTotal.do?tm='+new Date().getTime(),
    	data: encodeURI(""),
		dataType:'json',
		cache: false,
		success: function(data){
...
//下线
function goOut(msg){
	window.location.href=locat+"/fReeLogInLogout.do?tipsMessage="+msg;
}

//去通知收信人有站内信接收
function innerMsgNotice(USERNAME){
	var arrUSERNAME = USERNAME.split(';');
	for(var i=0;i<arrUSERNAME.length;i++){
		websocket.send('[InnerMsg]'+arrUSERNAME[i]);//发送通知
	}
}
...
function editPhoto(){
	 ...
	 diag.Drag=true;
	 diag.Title ="修改头像";
	 diag.URL = locat+'/headPanel/editPhoto.do';
	 diag.Width = 650;
	 diag.Height = 530;
...
function editSys(){
	 ...
	 diag.Drag=true;
	 diag.Title ="系统设置";
	 diag.URL = locat+'/headPanel/goSystem.do';
	 diag.Width = 600;
	 diag.Height = 526;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function innerMsg(){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = locat+'/fReeInnerMsg/listUserMsg.do?STATUS=2';
	 diag.Width = 800;
	 diag.Height = 500;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function changeMenus(){
	window.location.href=locat+'/fReeLayoutMain/yes';
}
