...
$(function(){if('createCode'== locat[3]){locat =  locat[0]+'//'+locat[2];}else{locat =  locat[0]+'//'+locat[2]+'/'+locat[3];};});
	...
	function save(){
		...
		for(var i=0;i<arField.length;i++){
			strArField = strArField + arField[i] + "Q416852186";
		}
	...
	function saveD(){
		...
		var uCamel = $("#uCamel").val(); //Michael新增 : var[7]把驼峰规则的属性名首字母变大写
		console.log(jdbcType);
		console.log(uCamel);
		if(dname==""){
			$("#dname").tips({
				side:3,
			...
			dname = dname.toLowerCase(); // toUpperCase();		//转化为大写
			if(isSame(dname)){
		...
		var fields = dname + ',fh,' + dtype + ',fh,' + dbz + ',fh,' + isQian + ',fh,' + ddefault + ',fh,' + flength + ',fh,' + jdbcType
		    + ',fh,' + uCamel;
	...
	function editField(value,msgIndex){
		...
		$("#jdbcType").val(efieldarray[6]); //Michael : var[6]改做 jdbcType
		$("#uCamel").val(efieldarray[7]);  //Michael新增 : var[7]把驼峰规则的属性名首字母变大写
	}
