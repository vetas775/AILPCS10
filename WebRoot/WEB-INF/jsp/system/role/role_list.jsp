...
<body class="no-skin">
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<table>
								...
								<td>
								<c:if test="${QX.edit == 1 }">
								<a class="btn btn-mini btn-info" onclick="editRole('${pd.roleId}','${pd.roleName}');">修改组名称<i class="icon-arrow-right  icon-on-right"></i></a>
								</c:if>
									<c:choose>
							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="margin-top:7px;">
								...
								<c:choose>
									<c:when test="${not empty roleList_z}">
										...
										<a class='btn btn-mini btn-info' title="编辑" onclick="editRole('${var.roleId}','${var.roleName}');"><i class='ace-icon fa fa-pencil-square-o bigger-130'></i></a>
										</c:if>
										<c:choose> 
	...
	<script type="text/javascript">
		...
		function editRole(roleId,roleName){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>role/toEditRole.do?roleId='+roleId+'&roleName='+roleName;
			 diag.Width = 222;
			 diag.Height = 100;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delRole(roleId,msg,roleName){
			bootbox.confirm("确定要删除["+roleName+"]吗?", function(result) {
				if(result) {
					var url = "<%=basePath%>role/deleteRole.do?roleId="+roleId
							+"&roleName="+roleName+"&guid="+new Date().getTime();
					top.jzts();
