...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="fReeInnerMsg/listUserMsg.do" method="post" name="Form" id="Form">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="padding-left:20px;"><a href="fReeInnerMsg/listUserMsg.do?TYPE=1"><span class="label label-<c:if test="${pd.TYPE != '2' }">success</c:if> arrowed-right arrowed-in">收信箱</span></a></td>
								<td><a href="fReeInnerMsg/listUserMsg.do?TYPE=2"><span class="label label-<c:if test="${pd.TYPE == '2' }">info</c:if> arrowed-right arrowed-in">发信箱</span></a></td>
							</tr>
						</table>
						<!-- 检索  -->
										...
										<tr>
											...
											<td class="center">
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${QX.BTN0001 == 1 }">
													<a class="btn btn-xs btn-info" title='发送站内信' onclick="goSendInnerMsg('${var.TO_USERNAME}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.BTN0001 == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="goSendInnerMsg('${var.TO_USERNAME}');" class="tooltip-info" data-rel="tooltip" title="发送站内信">
																	<span class="blue">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.BTN0001 == 1 }"><a title="批量发送站内信" class="btn btn-mini btn-info" onclick="makeAll('确定要给选中的用户发送站内信吗?');"><i class="ace-icon fa fa-envelope-o bigger-120"></i></a></c:if>
	...
	<script type="text/javascript">
		...
		//发站内信
		function goSendInnerMsg(username){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = '<%=basePath%>fReeInnerMsg/goAdd.do?username='+username;
			 diag.Width = 660;
			 diag.Height = 444;
		...
		function del(ztid,STATUS,type,Id,SANME_ID){
			bootbox.confirm("确定要删除吗?", function(result) {
				...
				if(result) {
					top.jzts();
					var url = "<%=basePath%>fReeInnerMsg/deleteOneMsg.do?FHSMS_ID="+Id+"&tm="+new Date().getTime();
		...
		function viewx(ztid,STATUS,type,Id,SANME_ID){
			if(type == "1" && STATUS == '2' && $("#"+ztid).html() == '<span class="label label-important arrowed-in">未读</span>'){
			 ...
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = '<%=basePath%>fReeInnerMsg/goView.do?FHSMS_ID='+Id+'&TYPE='+type+'&SANME_ID='+SANME_ID+'&STATUS='+STATUS;
			 diag.Width = 600;
			 diag.Height = 460;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>fReeInnerMsg/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
