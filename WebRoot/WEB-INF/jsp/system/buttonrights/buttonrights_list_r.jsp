...
<body class="no-skin">
									...
									<td style="width:50px;">角色组:</td>
										...
										<c:forEach items="${roleList}" var="role" varStatus="vs">
											<td style="width:100px;" class="center" <c:choose><c:when test="${pd.roleId == role.roleId}">bgcolor="#FFC926" onMouseOut="javascript:this.bgColor='#FFC926';"</c:when><c:otherwise>bgcolor="#E5E5E5" onMouseOut="javascript:this.bgColor='#E5E5E5';"</c:otherwise></c:choose>  onMouseMove="javascript:this.bgColor='#FFC926';" >
												<a href="buttonrights/list.do?roleId=${role.roleId }" style="text-decoration:none; display:block;"><i class="menu-icon fa fa-users"></i><font color="#666666">${role.roleName }</font></a>
											</td>
								...
								<tr>
									...
									<c:if test="${QX.edit == 1 }">
										...
										<c:forEach items="${roleList_z}" var="var" varStatus="vs">
											<th class='center'>${var.roleName }</th>
										</c:forEach>
									</c:if>
								</tr>
								</thead>
										...
										<c:if test="${QX.edit == 1 }">
											...
											<c:forEach items="${roleList_z}" var="var" varStatus="vs">
												...
												<c:forEach items="${roleFhbuttonlist}" var="varRb" varStatus="vsRb">
													<c:if test="${var.roleId == varRb.ROLE_ID && fhbutton.FHBUTTON_ID == varRb.BUTTON_ID }">
														<c:set value="1" var="rbvalue"></c:set>
													</c:if>
												</c:forEach>
												<td class='center' style="height: 20px;">
													<label>
														<input name="switch-field-1" onclick="upRb('${var.roleId}','${fhbutton.FHBUTTON_ID}')" class="ace ace-switch ace-switch-3" type="checkbox" <c:if test="${rbvalue == 1 }">checked="checked"</c:if> >
														<span class="lbl"></span>
													</label>
												</td>
	...
	<script type="text/javascript">
		$(top.hangge());		
		//处理按钮点击
		function upRb(roleId,FHBUTTON_ID){
			$.ajax({
				type: "POST",
				url: "<%=basePath%>buttonrights/upRb.do?ROLE_ID="+roleId+"&BUTTON_ID="+FHBUTTON_ID+"&guid="+new Date().getTime(),
		    	data: encodeURI(""),
				dataType:'json',
