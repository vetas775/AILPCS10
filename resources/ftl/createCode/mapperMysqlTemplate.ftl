...
<#list fieldList as var>
  ...
  <!--1 of 7 ${objectName}分页列表 -->
  <select id="listPage" parameterType="page" resultMap="BaseResultMap">
    select ${tablePk},<include refid="BaseColumnListWithoutPK"></include>
	from ${objectNameLower}
	<where>
      <if test="pd.keywords!= null and pd.keywords != ''"><!-- 关键词检索 -->
	    <!--	根据需求自己加检索条件
		字段1 LIKE "%"${r"#{pd.keywords}${r"}"}"%"
		or 
		字段2 LIKE "%"${r"#{pd.keywords}${r"}"}"%" 
		-->
	  </if>
