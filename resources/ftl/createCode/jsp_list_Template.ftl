			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="${objectNameLower}/list${objectName}.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="这里输入关键词" class="nav-search-input" id="keywords" autocomplete="off" name="keywords" value="${r"${pd.keywords}"}" placeholder="这里输入关键词"/>
								...
								<td style="vertical-align:top;padding-left:2px;">
									...
									<c:forEach items="${r"${listDate}"}" var="dateOne">
										<option value="${r"${dateOne.key}"}" <c:if test="${r"${pd.dateStyle==dateOne.key}"}">selected</c:if>>${r"${dateOne.value}"}</option>
									</c:forEach>
								  	</select>
								</td>
								<td ><font color="#808080">从</font></td>
								<td style="padding-left:2px;"><input class="span10 date-picker" name="dateFm" id="dateFm"  value="${r"${pd.dateFm}"}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="开始日期" title="开始日期"/></td>
								<td ><font color="#808080">到</font></td>
								<td style="padding-left:2px;"><input class="span10 date-picker" name="dateTo" id="dateTo"  value="${r"${pd.dateTo}"}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="结束日期"/></td>
								...
								<td style="vertical-align:top;padding-left:2px;">
									...
									<option value=""></option>
									<c:forEach items="${r"${listSort}"}" var="sortBy">
										<option value="${r"${sortBy.key}"}" <c:if test="${r"${pd.orderBy==sortBy.key}"}">selected</c:if>>${r"${sortBy.value}"}</option>
									</c:forEach>
								  	</select>
								</td>
								...
								<c:if test="${r"${QX.toExcel == 1}"}"><td style="vertical-align:top;padding-left:5px;"><a class="btn btn-light btn-xs" onclick="toExcel();" title="导出到EXCEL"><i id="nav-search-icon" class="ace-icon fa fa-download bigger-110 nav-search-icon green"></i></a></td></c:if>
							</tr>
						</table>
						<!-- 检索  -->
							...
							<c:choose>
								<c:when test="${r"${not empty varList}"}">
									...
									<c:forEach items="${r"${varList}"}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${r"${var."}${camelPk}${r"}"}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${r"${vs.index+1}"}</td>
										<#list fieldList as var>
										  <#if var[6] == 'TIMESTAMP'>
											<td class='center'><fmt:formatDate value="${r"${var."}${var[4]}${r"}"}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
										  <#else>
                                            <td class='center'>${r"${var."}${var[4]}${r"}"}</td>
                                          </#if>	
										</#list>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${r"${var."}${camelPk}${r"}"}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${r"${QX.del == 1 }"}">
													<a class="btn btn-xs btn-danger" onclick="del('${r"${var."}${camelPk}${r"}"}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${r"${QX.edit == 1 }"}">
															<li>
																<a style="cursor:pointer;" onclick="edit('${r"${var."}${camelPk}${r"}"}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${r"${QX.del == 1 }"}">
															<li>
																<a style="cursor:pointer;" onclick="del('${r"${var."}${camelPk}${r"}"}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
