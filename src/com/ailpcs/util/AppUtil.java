...
import com.ailpcs.util.fh.MapDistance;
/** 
 * 接口参数校验
 */
public class AppUtil  {
	...
	/**
	 * 检查某个接口调用的参数是否完整, 并在参数不完整时写error log
	 * @param method 被调用的接口名称
	 * @param pd 本次调用实际的请求参数
	 * @param reqParm 调用本接口需要用到的参数
	 * @return
	 */
	public static boolean checkParam(String method, PageData pd, String[] requiredParm) throws Exception {
		boolean result = false;
		int falseCount = 0;
		String[] tempArray  = new String[20];  //临时数组
		int size = requiredParm.length;
		for(int i=0;i<size;i++){
			String param = requiredParm[i];
			if(!pd.containsKey(param)){
				tempArray[falseCount] = param;
				falseCount += 1;
			}
		}
		if(falseCount>0){
	...
	/** 
	 * 当pd的key中包含"callback"时，把callback的值和map一起组成JSON对象然后返回; 否则直接返回map 
	 * @param pd
	 * @param map
	 * @return
	 */
