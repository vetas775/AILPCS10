...
import com.ailpcs.util.fh.ReflectHelper;
/**
 * MyBatis分页插件, 配置在mybatis-config.xml里
 * Amend by MichaelTsui 17/06/20
 */
@Intercepts({@Signature(type=StatementHandler.class,method="prepare",args={Connection.class})})
public class MyMybatisPagePlugin implements Interceptor {
		...
		if (parameterMappings != null) {
			...
			for (int i = 0; i < parameterMappings.size(); i++) {
				...
				if (parameterMapping.getMode() != ParameterMode.OUT) {
					...
					if (typeHandler == null) {
						throw new ExecutorException("[AILPCS]There was no TypeHandler found for parameter "+ propertyName + " of statement "+ mappedStatement.getId());
					}
