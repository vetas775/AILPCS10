...
public class SysDbBackupRecoveryController extends BaseController {
	private static final String menuUrl = "brdb/list.do"; 			//菜单地址(权限用)"数据还原"菜单
	private static final String menuUrlb = "brdb/listAllTable.do"; 	//菜单地址(权限用)"数据备份"菜单
	@Resource(name="brdbService")
	private SysDbBackupRecoveryService brdbService;
	...
	public Object backupAll(){
		...
		String kackupPath;
		try {
			kackupPath = DbUtil.getDbFH().backup("").toString();		//调用数据库备份
	...
	public Object backupTable(){
		...
		String kackupPath;
		try {
			kackupPath = DbUtil.getDbFH().backup(TABLENAME).toString();	//调用数据库备份
			if(Tools.notEmpty(kackupPath) && !"errer".equals(kackupPath)){
				pd.put("FHDB_ID", this.get32UUID());				//主键
				pd.put("USERNAME", username);						//操作用户
				pd.put("BACKUP_TIME", Tools.date2Str(new Date()));	//备份时间
	...
	public Object dbRecover(){
		...
		try {
			String returnStr = DbUtil.getDbFH().recover(TABLENAME, SQLPATH).toString();
