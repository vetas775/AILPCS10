...
import com.ailpcs.service.other.CreateCodeManager;
import com.ailpcs.util.AppUtil;
import com.ailpcs.util.CamelField;
import com.ailpcs.util.fh.DateUtil;
import com.ailpcs.util.fh.DelAllFile;
import com.ailpcs.util.fh.FileDownload;
import com.ailpcs.util.fh.FileZip;
import com.ailpcs.util.fh.Freemarker;
import com.ailpcs.util.fh.PathUtil;
...
public class CreateCodeController extends BaseController {	
	private static final String menuUrl = "createcode/list.do"; //菜单地址(权限用)
	...
	public void proCode(HttpServletResponse response) throws Exception{
		...
		String tabletop = pd.getString("tabletop");	   				//表主键				========参数3
		tabletop = null == tabletop?"":tabletop.toLowerCase();		//表主键转小写
		String zindext = pd.getString("zindex");	   	   			//属性总数
		String jdbcTypePk = pd.getString("jdbcTypePk"); //Michael17/07/08 记录主键的JDBC数据类型
		String parameterTypePK = pd.getString("parameterTypePK"); //Michael17/07/08 记录主键的Parameter Type数据类型	
		String pkGetter = pd.getString("pkGetter"); //Michael17/07/08 记录主键的getter事件名
		int zindex = 0;
		...
		root.put("tablePk", tabletop);								//表主键	
		root.put("camelPk", CamelField.underlineToCamel(tabletop));	//表主键转驼峰规则命名
		root.put("nowDate", new Date());							//当前日期
		root.put("jdbcTypePk", jdbcTypePk);  //Michael17/07/08 记录主键的JDBC数据类型
		root.put("parameterTypePK", parameterTypePK);  //Michael17/07/08 记录主键的Parameter Type数据类型	
		root.put("pkGetter", pkGetter);  //Michael17/07/08 记录主键的getter事件名
		...
		/*生成实体类*/
		Freemarker.printFile("entityTemplate.ftl", root, "entity/"+packageName+"/"+objectName+"DO.java", filePath, ftlPath);
		/*生成controller*/
		//Freemarker.printFile("controllerTemplate.ftl", root, "controller/"+packageName+"/"+objectName.toLowerCase()+"/"+objectName+"Controller.java", filePath, ftlPath);
		Freemarker.printFile("controllerTemplate2.ftl", root, "controller/"+packageName+"/"+objectName.toLowerCase()+"/"+objectName+"Controller.java", filePath, ftlPath);
		/*生成service*/
		Freemarker.printFile("serviceTemplate.ftl", root, "service/"+packageName+"/"+objectName.toLowerCase()+"/impl/"+objectName+"ServiceImpl.java", filePath, ftlPath);
		/*生成manager*/
		Freemarker.printFile("managerTemplate.ftl", root, "service/"+packageName+"/"+objectName.toLowerCase()+"/"+objectName+"Service.java", filePath, ftlPath);
		/*生成mybatis xml*/
		//Freemarker.printFile("mapperMysqlTemplate.ftl", root, "mybatis_mysql/"+packageName+"/"+objectName+"Mapper.xml", filePath, ftlPath);
		Freemarker.printFile("mapperMysqlTemplate2.ftl", root, "mybatis_mysql/"+packageName+"/"+objectName+"Mapper.xml", filePath, ftlPath);
		...
		Freemarker.printFile("jsp_list_Template2_useBootStrapTable.ftl", root, "jsp/"+packageName+"/"+objectName.toLowerCase()+"List.jsp", filePath, ftlPath);
		Freemarker.printFile("jsp_edit_Template.ftl", root, "jsp/"+packageName+"/"+objectName.toLowerCase()+"Detail.jsp", filePath, ftlPath);
		/*生成说明文档*/
	...
	public void save(PageData pd) throws Exception{
		...
		pd.put("jdbcTypePk", pd.getString("jdbcTypePk"));				//Michael17/07/08 记录主键的JDBC数据类型
		pd.put("parameterTypePK", pd.getString("parameterTypePK"));		//Michael17/07/08 记录主键的Parameter Type数据类型
		pd.put("pkGetter", pd.getString("pkGetter"));				    //Michael17/07/08 记录主键的getter事件名
